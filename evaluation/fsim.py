...
class FSIM_base(nn.Module):
    ...
    def phasecong2(self,img):
        ...
        # New version:
        imagefft = pt.fft.fft2(img)
        # Convert to old-style format [real, imag]
        ...
        # New version: #FIXME: 250224
        complex_input = pt.complex(filter_log_spread_zero.select(5,0), filter_log_spread_zero.select(5,1))
        ifftFilterArray = pt.fft.ifft2(complex_input).real * math.sqrt(rows*cols)
        ...
        # New version:
        complex_filter = pt.complex(filter_log_spread_repeat.select(5,0), filter_log_spread_repeat.select(5,1))
        complex_img = pt.complex(imagefft_repeat.select(5,0), imagefft_repeat.select(5,1))
        EO = pt.fft.ifft2(complex_filter * complex_img)
        EO = pt.stack([EO.real, EO.imag], dim=5)  # Convert back to old format
