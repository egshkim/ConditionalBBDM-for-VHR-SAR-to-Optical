...
class CW_SSIM(torch.nn.Module):
    ...
    def __init__(self, imgSize=[256,256], channels=3, level=4, ori=8, device = torch.device("cuda")):
        ...
        self.device = device
        print(f"\nInitializing CW-SSIM on device: {device}")
        self.ori = ori
        self.level = level
        ...
        # Move SP components to device
        for i in range(len(self.SP.indF)):
            self.SP.indF[i] = self.SP.indF[i].to(device)
            self.SP.indB[i] = self.SP.indB[i].to(device)
    ...
    def cw_ssim(self, x, y):
        ...
        for i in range(self.ori):
            ...
            band1 = cw_x[bandind][:,:,:,i,:,:].to(self.device)
            band2 = cw_y[bandind][:,:,:,i,:,:].to(self.device)
    ...
    def forward(self, x, y, as_loss=True, debug=False):
        # Move input tensors to the same device as the model
        x = x.to(self.device)
        y = y.to(self.device)
        if debug:
            print("\nCW-SSIM forward pass:")
            print(f"Input x device: {x.device}")
            print(f"Input y device: {y.device}")
            print(f"self.SP.indF[0] device: {self.SP.indF[0].device}")
        
        try:
            cw_x = self.SP(x)
            if debug:
                print("SP transform successful for x")
            cw_y = self.SP(y)
            if debug:
                print("SP transform successful for y")
            ...
            print(f"Error in CW-SSIM forward pass: {str(e)}")
